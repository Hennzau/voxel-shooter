...
pub struct ChunkMaterial {
    #[texture(0, dimension = "3d", filterable = true)]
...
impl Quad {
    pub fn from_direction(
        ...
        let x = (pos.x as u32).clamp(0, 15 as u32 - 1);
        let y = (pos.y as u32).clamp(0, 15 as u32 - 1);
        let z = (pos.z as u32).clamp(0, 15 as u32 - 1);

        let x1 = (x + 1).clamp(0, 15 as u32);
        let y1 = (y + 1).clamp(0, 15 as u32);
        let z1 = (z + 1).clamp(0, 15 as u32);
        ...
        let vertices = match direction {
            ...
            Direction::Front => {
                let normal = 5.clamp(0, 6);
                vec![
                    normal << 20 | 1 << 16 | 0 << 12 | z1 << 8 | y1 << 4 | x,
                    normal << 20 | 0 << 16 | 0 << 12 | z1 << 8 | y << 4 | x,
                    normal << 20 | 0 << 16 | 1 << 12 | z1 << 8 | y << 4 | x1,
                    normal << 20 | 1 << 16 | 1 << 12 | z1 << 8 | y1 << 4 | x1,
                ]
            }
        };
